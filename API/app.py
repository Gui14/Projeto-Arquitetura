# class Agendamento(db.Model):
#     id_agendamento = db.Column(db.Integer, primary_key=True, autoincrement=True)
#     data_atendimento = db.Column(db.Date)
#     horario_atendimento = db.Column(db.String(20))
#     id_doador = db.Column(db.Integer, db.ForeignKey('dados.id'))
#     nome_doador = db.Column(db.String(100))
#     hemocentro = db.Column(db.String(100) )
#     status = db.Column(db.String(20))
    
#     # Relacionamento com a tabela de usuários
#     doador = db.relationship('Dados', backref=db.backref('agendamentos', lazy=True))    


from flask import Flask, jsonify, request, make_response
from flask_sqlalchemy import SQLAlchemy
from flask_cors import CORS
from flask_bcrypt import Bcrypt
import jwt
import os
from datetime import datetime, timedelta
from werkzeug.utils import secure_filename

app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///database.db'
app.config['UPLOAD_FOLDER'] = './uploads/'
app.config['SECRET_KEY'] = 'seu_segredo_aqui'
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)

db = SQLAlchemy(app)
CORS(app, supports_credentials=True)
bcrypt = Bcrypt(app)

# Configurar CORS
CORS(app, supports_credentials=True, origins=["http://127.0.0.1:5500"])

class Dados(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    nome = db.Column(db.String(100))
    email = db.Column(db.String(100), unique=True)
    DataNasc = db.Column(db.String(20))
    senha = db.Column(db.String(100))
    foto_perfil = db.Column(db.String(200))

db.create_all()

@app.route('/usuarios/cadastrar', methods=['POST'])
def cadastrar_usuario():
    nome = request.form.get('nome')
    email = request.form.get('email')
    datanascimento = request.form.get('datanascimento')
    senha = request.form.get('senha')
    
    usuario_existente = Dados.query.filter_by(email=email).first()
    if usuario_existente:
        return jsonify({'error': 'E-mail já cadastrado'}), 400
    
    foto_perfil = None
    if 'foto_perfil' in request.files:
        foto = request.files['foto_perfil']
        if foto and foto.filename != '':
            foto_perfil_path = os.path.join(app.config['UPLOAD_FOLDER'], secure_filename(foto.filename))
            foto.save(foto_perfil_path)
            foto_perfil = foto.filename
    
    hashed_senha = bcrypt.generate_password_hash(senha).decode('utf-8')
    
    novo_dado = Dados(nome=nome, email=email, DataNasc=datanascimento, senha=hashed_senha, foto_perfil=foto_perfil)
    db.session.add(novo_dado)
    db.session.commit()

    return jsonify({
        'id': novo_dado.id,
        'nome': novo_dado.nome,
        'email': novo_dado.email,
        'datanascimento': novo_dado.DataNasc,
        'senha': novo_dado.senha,
        'foto_perfil': novo_dado.foto_perfil
    }), 201

@app.route('/usuarios/login', methods=['POST'])
def login():
    dados_login = request.get_json()
    email = dados_login.get('email')
    senha = dados_login.get('senha')
    
    usuario = Dados.query.filter_by(email=email).first()
    
    if usuario and bcrypt.check_password_hash(usuario.senha, senha):
        token = jwt.encode({
            'user_id': usuario.id,
            'exp': datetime.utcnow() + timedelta(hours=1)
        }, app.config['SECRET_KEY'], algorithm='HS256')

        # Definir o cookie de sessão
        response = make_response(jsonify({'message': 'Login realizado com sucesso'}), 200)
        response.set_cookie('session_token', token, httponly=True, samesite='Lax')
        response.headers.add('Access-Control-Allow-Origin', 'http://127.0.0.1:5500')
        response.headers.add('Access-Control-Allow-Credentials', 'true')
        return response
    else:
        return jsonify({'message': 'Credenciais inválidas'}), 401


@app.route('/usuarios/detalhes', methods=['GET'])
def detalhes_usuario():
    token = request.cookies.get('session_token')
    
    if not token:
        return jsonify({'message': 'Token inválido ou expirado'}), 401
    
    try:
        data = jwt.decode(token, app.config['SECRET_KEY'], algorithms=['HS256'])
        usuario = Dados.query.get(data['user_id'])
        
        if not usuario:
            return jsonify({'message': 'Usuário não encontrado'}), 404
        
        usuario_serializado = {
            'id': usuario.id,
            'nome': usuario.nome,
            'email': usuario.email,
            'DataNasc': usuario.DataNasc,
            'foto_perfil': usuario.foto_perfil
        }
        
        return jsonify(usuario_serializado), 200
    except jwt.ExpiredSignatureError:
        return jsonify({'message': 'Token expirado'}), 401
    except jwt.InvalidTokenError:
        return jsonify({'message': 'Token inválido'}), 401


    
#@app.route('/usuarios/cadastrar_usu', methods= ['POST'])
# def cadastrar_usuario_if():
#     novo_usuario = request.get_json()
#     #usuarios.append(novo_usuario)
#     usuario = Dados.query.filter_by(email = novo_usuario['Email'])
#     if usuario:
#         novo_dado = Dados(nome=novo_usuario['Nome'], cpf=novo_usuario['CPF'], DataNasc=novo_usuario['DataNasc'], email=novo_usuario['Email'], senha=novo_usuario['Senha'], TipoSangue=novo_usuario['TipoSang'], peso_atual = novo_usuario['Peso_atual'], pontos = novo_usuario['pontos'])
#         db.session.add(novo_dado)
#         db.session.commit()
#         return jsonify(novo_usuario) 
#     else:
#         return jsonify({'message': 'Email já existente'}), 404

# @app.route('/usuarios/excluir/<int:id>', methods = ['DEL'])
# def excluir_usuario(id):
#     usuario = Dados.query.get(id)
#     if usuario:
#         db.session.delete(usuario)
#         db.session.commit()
#         return jsonify({'message': 'Usuário excluído com sucesso'}), 200
#     else:
#         return jsonify({'message': 'Usuário não encontrado'}), 404    


# @app.route('/usuarios',methods=['GET'])
# @cross_origin()
# def obter_usuarios():
#     todos_os_dados = Dados.query.all()
#     usuarios_serializados = []
#     for dado in todos_os_dados:
#         usuario_serializado = {
#             'id': dado.id,
#             'nome': dado.nome,
#             'TipoSangue': dado.TipoSangue,
#             'email': dado.email,
#             'DataNasc': dado.DataNasc,
#             'cpf': dado.cpf,
#             'senha': dado.senha
#         }
#         usuarios_serializados.append(usuario_serializado)
#     return jsonify(usuarios_serializados)

# @app.route('/usuarios/<int:id>', methods=['GET'])
# def obter_usuario_id(id):
#     usuario = Dados.query.get(id)
#     if usuario:
#         usuario_serializado = {
#             'id': usuario.id,
#             'nome': usuario.nome,
#             'TipoSangue': usuario.TipoSangue,
#             'email': usuario.email,
#             'DataNasc': usuario.DataNasc,
#             'cpf': usuario.cpf,
#             'senha': usuario.senha
#         }
#         return jsonify(usuario_serializado)
#     else:
#         return jsonify({'message': 'Usuário não encontrado'}), 404

# @app.route('/usuarios/alterar', methods = ['PUT'])        
# def editar_usuario_id():
#     usuario_editado = request.get_json()
#     usuario = Dados.query.get(usuario_editado['Email'])
#     if usuario:
#         usuario.nome = usuario_editado.get('Nome',usuario.nome)
#         usuario.TipoSangue = usuario_editado.get('TipoSang', usuario.TipoSangue)
#         usuario.email = usuario_editado.get('Email', usuario.email)
#         usuario.DataNasc = usuario_editado.get('DataNasc', usuario.DataNasc)
#         usuario.cpf = usuario_editado.get('CPF', usuario.cpf)
#         usuario.senha = usuario_editado.get('Senha', usuario.senha)
#         db.session.commit()
#         return jsonify({'message': 'Usuário atualizado com sucesso'}), 200
#     else:
#         return jsonify({'message': 'Usuário não encontrado'}), 404 

# @app.route('/agendamento/<int:id_agendamento>', methods=['GET'])
# def obter_agendamento_id(id_agendamento):
#     # Buscar todos os agendamentos com o ID do doador correspondente
#     agendamentos = Agendamento.query.filter_by(id_doador=id_agendamento).all()
    
#     # Se houver agendamentos encontrados, serializar e retornar como JSON
#     if agendamentos:
#         agendamentos_serializados = []
#         for agendamento in agendamentos:
#             agendamento_serializado = {
#                 'id_agendamento': agendamento.id_agendamento,
#                 'data_agendamento': agendamento.data_atendimento.strftime('%Y%m%d'),
#                 'horario_agendamento': agendamento.horario_atendimento,
#                 'id_doador': agendamento.id_doador,
#                 'nome_doador': agendamento.nome_doador,
#                 'hemocentro': agendamento.hemocentro,
#                 'status': agendamento.status
#             }
#             agendamentos_serializados.append(agendamento_serializado)
        
#         return jsonify(agendamentos_serializados)
#     else:
#         return jsonify({'message': 'Nenhum agendamento encontrado para este doador'}), 404

# @app.route('/agendamento/atualizar/<int:id_agendamento>', methods=['PUT'])
# def atualizar_status(id_agendamento):
#     agendamento = Agendamento.query.get(id_agendamento)
#     usu_dados = Dados.query.get(agendamento.id_doador)
#     if agendamento:
#         agendamento.status = "Concluído"
#         usu_dados.pontos = 1 + usu_dados.pontos
#         db.session.commit()
#         return jsonify({'message': 'Status atualizado com sucesso'}), 200
#     else:
#         return jsonify({'message': 'Usuário não encontrado'}), 404 
    
# @app.route('/agendamento/cancelar/<int:id_agendamento>', methods=['PUT'])
# def atualizar_cancelar(id_agendamento):
#     agendamento = Agendamento.query.get(id_agendamento)
#     if agendamento:
#         agendamento.status = "Cancelado"
#         db.session.commit()
#         return jsonify({'message': 'Status atualizado com sucesso'}), 200
#     else:
#         return jsonify({'message': 'Usuário não encontrado'}), 404     

# @app.route('/agendamento/verificar/<string:data_agendamento>', methods=['GET'])
# def data_agendamento_data(data_agendamento):
#     data_agendamentos = Agendamento.query.filter_by(data_atendimento = data_agendamento).all()
#     print(data_agendamentos)
#     if data_agendamentos:
#         data_serializada = []
#         for data in data_agendamentos:
#             datas_serializada = {
#                 'horario_agendamento' :data.horario_atendimento
#             }
#             data_serializada.append(datas_serializada)
#         return jsonify(data_serializada)
#     else:
#         return jsonify({'message': "Não encontrado"})  
    
@app.route('/agendamento/peso/<int:id_agendamento>', methods=['PUT','OPTIONS'])
def atualizar_peso(id_agendamento):
    agendamento = Dados.query.get(id_agendamento)

    if agendamento:
        dados_agendamento = request.get_json()
        agendamento.peso_atual = dados_agendamento['Peso_atual']
        db.session.commit()  
        return jsonify({'message': 'Status atualizado com sucesso'}), 200
    else:
        return jsonify({'message': 'Usuário não encontrado'}), 404 
    
# @app.route('/agendamento/email/<string:email>', methods=['GET'])
# @cross_origin()
# def obter_agendamentos_por_email(email):
#     doador = Dados.query.filter_by(email=email).first()
#     if doador:
#         agendamentos = Agendamento.query.filter_by(id_doador=doador.id).all()
#         agendamentos_serializados = []
#         for agendamento in agendamentos:
#             agendamento_serializado = {
#                 'id_agendamento': agendamento.id_agendamento,
#                 'data_agendamento': agendamento.data_atendimento,
#                 'horario_agendamento': agendamento.horario_atendimento,
#                 'id_doador': agendamento.id_doador,
#                 'nome_doador': agendamento.nome_doador,
#                 'hemocentro': agendamento.hemocentro,
#                 'status': agendamento.status
#             }
#             agendamentos_serializados.append(agendamento_serializado)
#         return jsonify(agendamentos_serializados)
#     elif email == "*":
#         todos_os_dados = Agendamento.query.all()
#         agendamento_serializados = []
#         for agendamento in todos_os_dados:
#             agendamento_serializado = {
#                 'id_agendamento': agendamento.id_agendamento,
#                 'data_agendamento': agendamento.data_atendimento,
#                 'horario_agendamento': agendamento.horario_atendimento,
#                 'id_doador': agendamento.id_doador,
#                 'nome_doador': agendamento.nome_doador,
#                 'hemocentro': agendamento.hemocentro,
#                 'status' : agendamento.status
#             }
#             agendamento_serializados.append(agendamento_serializado)
#         return jsonify(agendamento_serializados)
#     else:
#         return jsonify({'message': 'Doador não encontrado'}), 404    
    
# @app.route('/agendamento/data/<string:email>', methods=['GET'])
# @cross_origin()
# def obter_agendamentos_por_data(email):
#     doador = Agendamento.query.filter_by(data_atendimento=email)
#     if doador:
#         agendamentos = Agendamento.query.filter_by(data_atendimento=email).all()
#         agendamentos_serializados = []
#         for agendamento in agendamentos:
#             agendamento_serializado = {
#                 'id_agendamento': agendamento.id_agendamento,
#                 'data_agendamento': agendamento.data_atendimento,
#                 'horario_agendamento': agendamento.horario_atendimento,
#                 'id_doador': agendamento.id_doador,
#                 'nome_doador': agendamento.nome_doador,
#                 'hemocentro': agendamento.hemocentro,
#                 'status': agendamento.status
#             }
#             agendamentos_serializados.append(agendamento_serializado)
#         return jsonify(agendamentos_serializados)
#     elif email == "*":
#         todos_os_dados = Agendamento.query.all()
#         agendamento_serializados = []
#         for agendamento in todos_os_dados:
#             agendamento_serializado = {
#                 'id_agendamento': agendamento.id_agendamento,
#                 'data_agendamento': agendamento.data_atendimento,
#                 'horario_agendamento': agendamento.horario_atendimento,
#                 'id_doador': agendamento.id_doador,
#                 'nome_doador': agendamento.nome_doador,
#                 'hemocentro': agendamento.hemocentro,
#                 'status' : agendamento.status
#             }
#             agendamento_serializados.append(agendamento_serializado)
#         return jsonify(agendamento_serializados)
#     else:
#         return jsonify({'message': 'Doador não encontrado'}), 404 

@app.route('/usuarios/pontos', methods = ['GET'])        
def pontuacao():
    usuario_pontos = request.get_json()
    usuario = Dados.query.get(usuario_pontos['id'])
    pontos_serializado = []
    
    if usuario:
        pontos_serializado = {
            'pontos': usuario.pontos
        }
        return jsonify(pontos_serializado)
    else:
        return jsonify({'message': 'Usuário não encontrado'}), 404     
    
app.run(port=5000, host='192.168.0.103', debug=True)

